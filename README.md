...
5. [征文正文合并稿](docs/draft/征文正文合并稿.md)：当前推荐审阅稿，以面向评委的叙事为主，已融合技术机制、案例事实和证据组织。
6. [征文正文初稿](docs/draft/征文正文初稿.md)：技术细节和待补数据较完整的早期工作稿，作为合并稿的事实与数据补充来源。
...
当前已形成征文正文合并稿。下一阶段应补齐统一口径的案例数据、平台运行指标和脱敏证据，并根据申报平台的实际篇幅限制完成文字压缩。
